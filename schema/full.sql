...
--------------------------------------------------------------------------------
-- What the drafting agent worked out about a project before writing any
-- objective, and then had to either cover or decline [added in 054].
--
-- A brief describes a mechanism, and objectives drafted straight from one are
-- that mechanism restated. These are the two enumerations that get above it,
-- both derived from the project rather than carried by Mendel as a checklist:
--
--   failure_mode  a way this project fails with the software working exactly as
--                 described. Stipulating that the code works is what excludes
--                 the mechanism, which the brief already said.
--   party         someone or something the system exchanges with, whose
--                 experience decides whether this succeeds -- a person, another
--                 program, an agent, a supplier whose outage is
--                 indistinguishable from your own. For a party that cannot
--                 complain, a bad experience is latency, error semantics and
--                 uptime, so software quality arrives through this enumeration
...
CREATE TABLE strategic_considerations (
    ...
    -- hand. Exactly one is set: the objective covering it, or the reason
    -- nothing does. Both null means no draft has judged it yet -- a third
    -- state, and not the same as uncovered.
    --
    -- SET NULL rather than CASCADE because a redraft deletes every objective
    -- and writes new ones: the consideration survives that, its coverage does
    -- not, and the next pass decides again.
