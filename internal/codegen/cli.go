...
func BuildImplementationPrompt(hopName, variationName, approach, testCommand string) string {
	...
	prompt.WriteString("- Test idempotency if applicable (running migration twice shouldn't fail)\n")
	prompt.WriteString("- Place tests alongside the migration or in the project's test directory\n\n")
	prompt.WriteString("### 3. Create `.mendel/migration.json` for temporary demo/testing\n")
	prompt.WriteString("This file contains instructions to apply/revert schema changes during demo:\n")
	prompt.WriteString("```json\n")
	prompt.WriteString("{\n")
	prompt.WriteString("  \"up_instructions\": \"Instructions to apply the migration. Reference MENDEL.md or specify commands.\",\n")
	prompt.WriteString("  \"down_instructions\": \"Instructions to cleanly revert. Must NOT affect unrelated data.\",\n")
	prompt.WriteString("  \"notes\": \"Path to migration files in the CODE repo, e.g. 'db/migrations/20240101_add_users.sql'\"\n")
	prompt.WriteString("}\n")
	prompt.WriteString("```\n")
	prompt.WriteString("- The temporary migration is reverted when the variation ends (win or lose)\n")
	prompt.WriteString("- The real migration runs through normal deployment after merge\n\n")
	prompt.WriteString("### 4. If no schema changes needed, skip all migration steps\n")

	prompt.WriteString("\n## Demo Configuration\n\n")
	prompt.WriteString("If this variation changes how the service is started, deployed, or run:\n\n")
	prompt.WriteString("Create or update `.mendel/demo.yaml` with deployment instructions:\n\n")
	prompt.WriteString("```yaml\n")
	prompt.WriteString("version: 1\n")
	prompt.WriteString("type: local  # or \"cloud\" for remote deployments\n")
	prompt.WriteString("\n")
	prompt.WriteString("# Command to start the service (required)\n")
	prompt.WriteString("# Variables: ${PORT}, ${VARIATION_ID}, and for cloud: ${DEPLOY_URL}\n")
	prompt.WriteString("start: \"npm run dev\"\n")
	prompt.WriteString("\n")
	prompt.WriteString("# Command to stop the service (required)\n")
	prompt.WriteString("stop: \"pkill -f 'node.*${PORT}'\"\n")
	prompt.WriteString("\n")
	prompt.WriteString("# URL to check for health (required)\n")
	prompt.WriteString("health_url: \"http://localhost:${PORT}/health\"\n")
	prompt.WriteString("\n")
	prompt.WriteString("# Optional: port configuration (local mode only)\n")
	prompt.WriteString("port:\n")
	prompt.WriteString("  default: 3000\n")
	prompt.WriteString("  range: 100\n")
	prompt.WriteString("\n")
	prompt.WriteString("# Optional: commands to run before start\n")
	prompt.WriteString("setup:\n")
	prompt.WriteString("  - \"npm install\"\n")
	prompt.WriteString("\n")
	prompt.WriteString("# Optional: file to copy to .env before start\n")
	prompt.WriteString("env_file: \".env.example\"\n")
	prompt.WriteString("\n")
	prompt.WriteString("# Cloud mode only: regex to extract URL from start command output\n")
	prompt.WriteString("# If empty, looks for first https:// URL in output\n")
	prompt.WriteString("url_pattern: \"\"\n")
	prompt.WriteString("```\n\n")
	prompt.WriteString("Only create/update this file if the variation affects how the service runs.\n")
	prompt.WriteString("Skip this section if the variation is purely code/logic changes.\n")
