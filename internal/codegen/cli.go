...
func BuildImplementationPrompt(hopName, variationName, approach, testCommand string) string {
	...
	prompt.WriteString("### 1. `.mendel/docker-compose.yml`\n\n")
	prompt.WriteString("This defines all services needed to run the demo:\n\n")
	prompt.WriteString("```yaml\n")
	prompt.WriteString("# If the project already has a docker-compose.yml, include it:\n")
	prompt.WriteString("include:\n")
	prompt.WriteString("  - path: ../docker-compose.yml\n")
	prompt.WriteString("\n")
	prompt.WriteString("# Or define services directly:\n")
	...
	prompt.WriteString("      test: [\"CMD\", \"curl\", \"-f\", \"http://localhost:3000/health\"]\n")
	...
	prompt.WriteString("\n")
	prompt.WriteString("# Optional: commands to run after containers are up\n")
	prompt.WriteString("after_up:\n")
	prompt.WriteString("  - \"docker-compose exec app npm run migrate\"\n")
	prompt.WriteString("  - \"docker-compose exec app npm run seed\"\n")
	prompt.WriteString("```\n\n")
	prompt.WriteString("Skip demo configuration if the variation is purely code/logic changes with no deployment impact.\n")
